  ...
  return (
    <main className="min-h-screen bg-gray-50 pb-24">
      <div className="mx-auto w-full max-w-2xl px-5 py-7 sm:px-8">
        ...
        {step === 2 && <section className="mt-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-bold">Upload documents</h2><p className="mt-2 text-sm leading-6 text-gray-600">PDF or JPEG only, up to 5 MB per file. Saving this step submits the application because documents must be attached to an application ID.</p><div className="mt-5 space-y-4">{documents.map((document, index) => { const isSuccess = document.status === 'success'; const isUploading = document.status === 'uploading' || uploading; return <div className={`rounded-xl border p-4 ${document.status === 'error' ? 'border-red-300 bg-red-50' : isSuccess ? 'border-green-300 bg-green-50' : 'border-gray-200'}`} key={document.id}><div className="flex items-center justify-between gap-3"><p className="text-sm font-bold">Document {index + 1} {isSuccess && <span className="text-green-700">(Uploaded)</span>} {document.status === 'error' && <span className="text-red-700">(Failed)</span>}</p><button className="min-h-10 text-sm font-semibold text-red-700 underline disabled:text-gray-400" type="button" onClick={() => removeDocument(document.id)} disabled={isUploading || isSuccess}>Remove</button></div><label className="mt-3 block text-sm font-semibold">Document type<select className="mt-1 block min-h-11 w-full rounded-lg border border-gray-300 bg-white px-3 disabled:bg-gray-100" value={document.docType} onChange={(event) => changeDocument(document.id, 'docType', event.target.value)} disabled={isUploading || isSuccess}>{Object.entries(documentLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label className="mt-3 block text-sm font-semibold">File<input className="mt-1 block w-full text-sm text-gray-700 file:mr-3 file:rounded-lg file:border-0 file:bg-gray-100 file:px-3 file:py-2 file:text-sm file:font-semibold disabled:opacity-50" type="file" accept=".pdf,.jpg,.jpeg,application/pdf,image/jpeg" onChange={(event) => changeDocument(document.id, 'file', event.target.files?.[0] || null)} disabled={isUploading || isSuccess} /></label>{document.file && <p className="mt-2 text-xs text-gray-600">Selected: {document.file.name} ({Math.ceil(document.file.size / 1024)} KB)</p>}</div>})}</div><button className="mt-4 min-h-11 rounded-lg border border-gray-300 px-4 text-sm font-semibold disabled:text-gray-400" type="button" onClick={addDocument} disabled={uploading}>Add a document</button>{uploading && <div className="mt-4" role="status"><p className="text-sm font-semibold">Uploading… {progress}%</p><progress className="mt-2 h-2 w-full accent-black" max="100" value={progress}> {progress}% </progress></div>}<div className="mt-6 flex gap-3"><button className="min-h-12 flex-1 rounded-xl border border-gray-300 px-4 text-sm font-bold disabled:text-gray-400" type="button" onClick={() => setStep(1)} disabled={uploading || Boolean(applicationId)}>Back</button><button className="min-h-12 flex-[2] rounded-xl bg-black px-4 text-sm font-bold text-white disabled:bg-gray-400" type="button" onClick={saveAndUpload} disabled={uploading}>{uploading ? 'Saving…' : documents.some((d) => d.status === 'error') ? 'Retry failed' : 'Save and review'}</button></div>{applicationId && <p className="mt-3 text-xs leading-5 text-gray-600">Your application is already submitted. You can add documents here, but the backend does not support editing personal information after submission.</p>}</section>}
